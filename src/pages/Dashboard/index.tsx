import { Search, RefreshCcw, Sigma, Percent, ChartLine, Bell } from 'lucide-react'
...
export default function Dashboard() {
  const {searchTerm, setNoReadCount, notifyOpen, setNotifyOpen, isPc} = useContext(DashboardContext);
  ...
  const statistics = useMemo(() => {
    ...
    const average = dustbinData.reduce((acc, bin) => acc + bin.fullness, 0) / total || 0;
  ...
  const onScroll = useCallback((e) => {
    ...
    const upDomHeight = e.target.firstChild?.firstChild?.firstChild?.offsetHeight + 64;
  ...
  useEffect(() => {
    if (rootRef.current) {
      rootRef.current.parentElement?.addEventListener('scroll', onScroll, { passive: true });
      return () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex-1 w-full" ref={rootRef}>
      {/* Main Content */}
      <main className="container mx-auto px-4 py-4">
          ...
          <Card className="bg-white/50 backdrop-blur-sm border-0 shadow-lg hover:shadow-xl transition-shadow">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <div className="flex flex-row items-center justify-between pb-2 w-full">
                <CardTitle className="text-lg font-medium text-gray-500">Total Dustbins</CardTitle>
            ...
            <CardContent>
              ...
              <p className="text-md text-gray-500 mt-1">Active dustbins in your network</p>
            </CardContent>
          </Card>
          <Card className="bg-white/50 backdrop-blur-sm border-0 shadow-lg hover:shadow-xl transition-shadow">
            <CardHeader>
              <div className="flex flex-row items-center justify-between pb-2 w-full">
                <CardTitle className="text-lg font-medium text-gray-500">Average Fullness</CardTitle>
                <div className="p-2 bg-primary/10 rounded-full">
            ...
            <CardContent>
              ...
              <p className="text-md text-gray-500 mt-1">Current average capacity</p>
            </CardContent>
          </Card>
        </div>

        {/* Dustbin Categories Filter */}
        <Flex className={clsx('gap-2 mb-8', { 'absolute top-[73px] left-0 bg-white/50 p-4 z-10': !isPc && filterFixed })}>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {categories.filter(c => c.fullness >= selectedFilter).map((oc) => {
            const category = {
              ...oc,
              color: oc.fullness > FULLNESS_THRESHOLD ? COLORS.high : oc.color
            };
            const chartOpts = {
              ...chartOptions,
              ...
              >
                <CardHeader>
                  <div className="flex flex-row items-center justify-between">
                    ...
                    <Button ghost type='text' className="rounded-full" onClick={() => setDetail(category)}>
        ...
        <Drawer
          ...
          closeIcon={null}
          extra={[
            <Button type='text' key={'export'} onClick={exportNotifications} icon={<DownloadOutlined />}>Export</Button>,
            <Button type='text' key={'close'} onClick={() => setNotifyOpen(false)} icon={<CloseOutlined />} />,
          ]}
        >
